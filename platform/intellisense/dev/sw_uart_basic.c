...
SW_UARTInitTxRxTimers(unsigned long ulClkFreq)
{
    ...
    IntPrioritySet(INT_TIMER1B, 0x10);
    ...
    // Used for TX.
    //
    TimerLoadSet(TIMER1_BASE, TIMER_A, g_ulTxBaudTimerTickValue);

    //
    // Used for RX, 1uS offset to account for interrupt delays.
    //
    TimerLoadSet(TIMER1_BASE, TIMER_B, (g_ulRxBaudTimerTickValue -
                                        (ulClkFreq/1000000)));
...
SW_UARTStartBitDetect(void)
{
    ...
    if( ulPortIntsStatus & SWU_GPIO_RX)
    {
        ...
        //
        HWREG(TIMER1_BASE + TIMER_O_TBILR) = ( g_ulTxBaudTimerTickValue +
                                             g_ulRxBaudTimerTickValue - 200 );

        //
        // Used for RX, 1uS offset to account for interrupt delays
        //
        HWREG(TIMER1_BASE + TIMER_O_CTL) |= TIMER_B & (TIMER_CTL_TAEN |
                                                       TIMER_CTL_TBEN);
    }
}
        ...
        case SWUART_STOP_1:
        {
            ...
            if(ulRcvIoValue)
            {
                ...
                if(g_sSoftwareUART.StopBits == SWUART_STOP_ONE)
                {
                	...
                	serial_input(&g_ulRxChar);
                    //
                    // Disable the receiver timer
                    //
                    HWREG(TIMER1_BASE + TIMER_O_CTL) &=
...
SW_UARTCharPut(unsigned long ulBase, unsigned char ucData)
{
    ...
    //
    while(!(SW_UARTCharPutNonBlocking(0,ucData)))
    {
    }
}
